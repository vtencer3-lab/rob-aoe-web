...
// Taunt 11 (smích) zní jako ve hře: příjemci místo cinknutí a autorovi taky.
it("taunt 11 zahraje smích příjemci i autorovi, obyčejná zpráva cinkne", async () => {
  vi.mocked(api.me).mockResolvedValue({ hrac: { steamId: "b", alias: "Spoluhrac", steamName: null, jeAdmin: false } });
  const bez = { ...zapas([u("a", 1, 1, true), u("b", 2, 2)]), zpravy: [] };
  nastavStav({ akce: { id: 1, nazev: "Akce 1", stav: "bezi" }, prihlaseni: [], zapasy: [bez] });
  const { rerender } = render(<App />);
  await screen.findByTestId("chat");
  const cizi = { id: 1, steamId: "a", jmeno: "Host", jeAdmin: false, barva: 1 as const, tym: 1 as const, text: "11", poslano: "2026-09-12T12:00:00.000Z" };
  nastavStav({ akce: { id: 1, nazev: "Akce 1", stav: "bezi" }, prihlaseni: [], zapasy: [{ ...bez, zpravy: [cizi] }] });
  rerender(<App />);
  await vi.waitFor(() => expect(prehraj).toHaveBeenCalledTimes(1));
  expect(String(vi.mocked(prehraj).mock.calls[0]![0])).toMatch(/smich/);
  ...
  expect(String(vi.mocked(prehraj).mock.calls[1]![0])).toMatch(/smich/);
